...
def alarm_reduce_rate(unit, before_path, rules_path):
	before, rules_path_unit = before_path + unit, rules_path + unit
	o1, o2 = open(before, 'rb'), open(rules_path_unit, 'rb')
	df_before, df_rules = pd.read_csv(o1), pd.read_csv(o2)
	count_before = len(df_before)
	rules = df_rules['rules'].as_matrix()
	# print(count_before, rules[0])
	count_after = 0
	for idx in df_before.index:
		start_time = df_before.loc[idx]['start_time']
