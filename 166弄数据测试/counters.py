...
#  	d_path：结果存储的路径；
#  】
def rooms_open_counter(unit, o_path, d_path):
	path = o_path + unit
	o = open(path, 'rb')
	...
	df_out.to_csv(d_path + unit, index=None)
...
#  	o_path: 源数据路径；
#  】
def total_open_condition(unit, df_out, i, o_path):
	path = o_path + unit
	o = open(path, 'rb')
	df = pd.read_csv(o, parse_dates=['received_time'], date_parser = dateparse2)
	address = unit.split('.')[0]
	...
	for idx in df.index:
		...
		if df.loc[idx]['status'] == 4:
			...
			# 周末日间
				...
				weekend_daytime += 1
			# 工作日工作时间
			elif received_time.hour >= 8 and received_time.hour <= 18:
				workday_workhours += 1
			# 工作日通勤时间
			else:
		...
		if idx == df.index[-1]:
			df_out.loc[i] = { "address": address, 'workday_workhours': workday_workhours, 'workday_commuting_hours': workday_commuting_hours, 'weekend_daytime': weekend_daytime, 'wee_hours':wee_hours }
...
#  	d_path：结果存储的路径；
#  】
def open_frequency_peer_hour(unit, o_path, d_path):
	path = o_path + unit
	o = open(path, 'rb')
	...
	csvfile = open(d_path + unit, 'w')
	df_out.to_csv(d_path + unit, index=None, encoding='utf-8')
...
#  	d_path：结果存储的路径；
#  】
def open_frequency_peer_day(unit, o_path, d_path):
	path = o_path + unit
	o = open(path, 'rb')
