...
#  	d_path：结果存储的路径；
#  】
def model(address, unit, o_path, d_path):
	# 读取数据
	df = read_df(address, unit, o_path, d_path)
	if len(df) == 0:
		return
	...
	# 保存模型数据
	new_path = d_path + unit + '/csv/'
	if not os.path.exists(new_path):
		os.makedirs(new_path)
	csv_file = open(new_path + address, 'w')
	model_df.to_csv(new_path + address, index=None)
	...
	fig.suptitle(address_name)
	new_path = d_path + unit + '/img/'
	if not os.path.exists(new_path):
		os.makedirs(new_path)
	plt.savefig(new_path + address_name + '.png')
	plt.close()
